...
export default class Unit {
  ...
  takeDamages(damages, skill_type, round, name) {
    let current = '';
    if (this.name === 'attacker') current = 'D';
    else current = 'A';
    let currentlog = `[${this.name.substring(0, 1).toUpperCase()}] ${this.key} (${this.i}) with ${
      this.health
    } HP take <span style="color:red">${damages} DMG</span> from [${current}] ${name} with <span style="color:blueviolet"> "${skill_type}"</span>.`;
    ...
    if (this.skill.type === 'dodge' && this.use > 0 && damages > this.health) {
      ...
      currentlog += ` [${this.name.substring(0, 1).toUpperCase()}] ${this.key} (${
        this.i
      }) got now ${this.health} HP.`;
      this.log.add(currentlog);
    } else {
      currentlog += ` [${this.name.substring(0, 1).toUpperCase()}] ${this.key} (${
        this.i
      }) is <span style="color:darkorange">now dead.</span>`;
      this.log.add(currentlog);
      this.kill();
    }
  }
  ...
  takeBuff(points, skill_type, round, name) {
    if (
      (this.health < this.max_health && skill_type === 'heal') ||
      (this.health < this.max_health && skill_type === 'groupheal')
    ) {
      this.log.add(
        ` [${this.name.substring(0, 1).toUpperCase()}] ${this.key} (${
          this.i
        }) take ${skill_type} <span style="color:chartreuse">+${points} HP</span> from ${name}.`,
      );
