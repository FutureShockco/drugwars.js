...
export default class Army {
  constructor(units, name, trainings, buildings, log) {
    ...
    this.groups.forEach(unit => {
      // HOBO
      if (unit.key === "hobo") {
        const kamikaze = this.trainings.find(b => b.key === 'spiritwine' || b.training === 'spiritwine');
        if (kamikaze)
          unit.attack = unit.attack + (unit.attack / 100 * kamikaze.lvl);
      }
      else {
        const protection = this.trainings.find(b => b.key === 'protection' || b.training === 'protection');
        if (protection)
          unit.defense = unit.defense + (unit.defense / 5 * protection.lvl);
        //ALL MELEE
        if (unit.spec.type === 'Melee') {
          const closecombat = this.trainings.find(b => b.key === 'closecombat' || b.training === 'closecombat');
          if (closecombat)
            unit.attack = unit.attack + (unit.attack / 100 * closecombat.lvl);
        }
        else {
          const firearms = this.trainings.find(b => b.key === 'firearms' || b.training === 'firearms');
          if (firearms) {
            unit.attack = unit.attack + (unit.attack / 100 * firearms.lvl);
          }
        }
        if (unit.key === "sniper") {
          const sniping = this.trainings.find(b => b.key === 'sniping' || b.training === 'sniping');
          if (sniping)
            unit.attack = unit.attack + (unit.attack / 100 * sniping.lvl);
        }
        else if (unit.key === "bazooka") {
          const bomb = this.trainings.find(b => b.key === 'bomb' || b.training === 'bomb');
          if (bomb)
            unit.attack = unit.attack + (unit.attack / 100 * bomb.lvl);
        }
        //WEAPON
        else if (unit.key === "rowdy" || unit.key === "sniper" || unit.key === "hitman" || unit.key === "biker") {
          const firearms = this.trainings.find(b => b.key === 'weapon' || b.training === 'weapon');
          if (firearms) {
            unit.attack = unit.attack + (unit.attack / 100 * firearms.lvl);
          }
        }
        ...
        if (unit.key === "bazooka" || unit.key === "gunman" || unit.key === "helicopter") {
          const fire = this.trainings.find(b => b.key === 'fire' || b.training === 'fire');
          if (fire)
        ...
        if (unit.key === "mercenary" || unit.key === "knifer") {
          const chemical = this.trainings.find(b => b.key === 'chemical' || b.training === 'chemical');
          if (chemical)
        ...
        if (unit.key === "mercenary" || unit.key === "knifer" || unit.key === "big_mama" || unit.key === "ninja") {
          const psychological = this.trainings.find(b => b.key === 'psychological' || b.training === 'psychological');
          if (psychological) {
  ...
  attackPower(trainings) {
    ...
    const coordination = trainings.find(b => b.key === 'coordination' || b.training === 'coordination');
    if (coordination)
